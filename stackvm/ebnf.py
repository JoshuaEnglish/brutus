...
class EBNFParser(object):
    ...
    def match_nonterminal_new(self, ebnfnode, list_of_tokens, i):
        """Revamp of match_nontermnial"""
        found = []
        self._report(i, "NEW NON_TERM MATCH for %s" % ebnfnode)
        this_min = 0 if ebnfnode.optional or ebnfnode.repeating else 1
        try_repeat = ebnfnode.repeating or ebnfnode.oneormore 
        self._report(i, "mntn:this_min is %s" % this_min)
        self._report(i, "mntn:try_repeat is %s" % try_repeat)
        if ebnfnode.alternate:
            potential_sequences = split_by_or(ebnfnode.children)
            original_tokens = list(list_of_tokens) # create a copy
            
            while potential_sequences:
                self._report(i, "mntn:trying alternate %s" % token_lexemes(potential_sequences[0]))
                found, remaining_tokens = self.match_sequence(ebnfnode.token.lexeme, 
                                                              potential_sequences[0], 
                                                              list_of_tokens, i+1)
                if found:
                    break
                potential_sequences = potential_sequences[1:]
        else:
            found, remaining_tokens = self.match_sequence(ebnfnode.token.lexeme, 
                                                          ebnfnode.children, 
                                                          list_of_tokens, i+1)
        self._report(i, "mntn:found %s" % token_lexemes(found))
        
        node = CSTNode(ebnfnode.token)
        if not found: # no alternates or sequences found
            if this_min == 0:
                return CSTNode(ebnfnode.token), list_of_tokens
            #else:
                #raise ValueError('Failed parsing %s', ebnfnode.token.lexeme)
        else:
            
            node.children.extend(found)
            if try_repeat:
                while True:
                    next_sequence, remaining_tokens = self.match_sequence(ebnfnode.token.lexeme,
                                                                          ebnfnode.children,
                                                                          remaining_tokens, i+1)
                    if next_sequence:
                        node.children.extend(next_sequence)
                    else:
                        break
        return node, remaining_tokens
            
### 2016-12-10 STATE MACHINE ATTEMPT
